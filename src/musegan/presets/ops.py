...
CONV_KERNEL_INITIALIZER = tf.compat.v1.truncated_normal_initializer(stddev=0.05)
DENSE_KERNEL_INITIALIZER = tf.compat.v1.truncated_normal_initializer(stddev=0.05)

#dense = lambda i, u: tf.layers.dense(
dense = lambda i, u: tf.compat.v1.layers.dense(
...
conv3d = lambda i, f, k, s: tf.compat.v1.layers.conv3d(
    i, f, k, s, kernel_initializer=CONV_KERNEL_INITIALIZER)
tconv2d = lambda i, f, k, s: tf.layers.conv2d_transpose(
    i, f, k, s, kernel_initializer=CONV_KERNEL_INITIALIZER)
#tconv3d = lambda i, f, k, s: tf.layers.conv3d_transpose(
tconv3d = lambda i, f, k, s: tf.compat.v1.layers.conv3d_transpose(
    i, f, k, s, kernel_initializer=CONV_KERNEL_INITIALIZER)
...
def get_normalization(norm_type, training=None):
    ...
    if norm_type == 'batch_norm':
        ...
        return lambda x: tf.compat.v1.layers.batch_normalization(x, training=training)
